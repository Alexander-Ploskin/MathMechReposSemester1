...
void addVertex(ListOfVertexes* list, int distance, int vertex, int adjacentVertex)
{
	if (isEmpty(list))
	{
		list->head = new ListElement{ vertex, distance, list->head, adjacentVertex };
		return;
	}
	...
	while (currentElement->next != nullptr)
	{
		if (currentElement->vertex == vertex)
		{
			...
			{
				currentElement->adjacentVertex = adjacentVertex;
	...
	currentElement->next = new ListElement{ vertex, distance, nullptr, adjacentVertex };
}
