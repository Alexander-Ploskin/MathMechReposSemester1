...
bool add(Set* set, int distance, int vertex, int adjacentVertex)
{
	if (contains(set, distance))
	{
		return false;
	}
	...
	if (isEmpty(set))
	{
		set->root = new SetElement{ nullptr, nullptr, nullptr, distance, vertex, adjacentVertex };
		return true;
	}
	...
	while (true)
	{
		if (currentElement->distance < distance)
		{
			...
			{
				currentElement->rightChild = new SetElement{ nullptr, nullptr, currentElement, vertex, adjacentVertex };
		...
		else
		{
			...
			{
				currentElement->leftChild = new SetElement{ nullptr, nullptr, currentElement, distance, vertex, adjacentVertex };
...
int getMininumInSetAndRemove(Set* set, int& adjacentVertex)
{
	...
	adjacentVertex = currentElement->adjacentVertex;
	int result = currentElement->vertex;
	remove(set, currentElement->distance);
	return result;
}